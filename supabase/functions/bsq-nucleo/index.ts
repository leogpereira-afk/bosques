// ============================================================================
// Edge Function "bsq-nucleo" — backend do Portal dos Bosques (loteadora).
...
Deno.serve(async (req) => {
  ...
  if (body.action === "list" || body.action === "getCfg") {
    const ROTINA = Deno.env.get("BSQ_ROTINA_TOKEN");
  ...
  try {
    switch (action) {
      ...
      case "entrar": {
        ...
        if (!esperado) return json({ ok: false, error: "Sistema ainda não configurado: falta o segredo BSQ_PAINEL_SENHA." }, 503);
        if (!autenticado) {
      ...
      case "esvaziarLixeira": {
        const { data } = await db.from("bsq_registros").select("colecao, id, registro").eq("apagado", true);
      ...
      case "reiniciarNumeracao": {
        ...
        const { data } = await db.from("bsq_seq_idx").delete().eq("colecao", col).gte("numero", proximo).select("numero");
    ...
    console.error("[bsq-nucleo] erro:", e);
