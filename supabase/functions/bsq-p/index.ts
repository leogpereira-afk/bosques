// ============================================================================
// Edge Function "bsq-p" — a página que o CLIENTE abre pelo link do WhatsApp.
//
//   GET  /bsq-p/<idProposta>/<token>          → landing (teaser + botões)
//   GET  /bsq-p/<idProposta>/<token>?pdf=1    → o PDF da proposta
//   POST /bsq-p/<idProposta>/<token>          → registra "tenho interesse"/"dúvida"
//
...
Deno.serve(async (req) => {
  ...
  const iFn = partes.indexOf("bsq-p");
  ...
  const selfUrl = (Deno.env.get("SUPABASE_URL") || "") + "/functions/v1/bsq-p/" + id + "/" + t;
