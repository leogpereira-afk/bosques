...
> este documento é o suficiente. Escrito em 27/08/2026 (versão v42 do app).
...
`leogpereira-afk/bosques`), falando com **Edge Functions** do Supabase (projeto
compartilhado "Projetos Léo", ref `reoghclxripktzpdwhiy`, namespace `bsq_*`),
...
| Tabela | Papel |
|---|---|
| `bsq_registros (colecao, id, registro jsonb, atualizado_em, apagado)` | o cofre: TODA coleção vive aqui como JSONB |
| `bsq_cfg (id=true, config jsonb)` | linha única: empresa, regras, usuários (hashes nunca saem) |
| `bsq_seq` + `bsq_seq_idx` + fn `bsq_proximo_numero()` | numeração atômica (VD-0001, PR-0001, RB-0001) |
| `bsq_log` | auditoria de ações |
| `bsq_backup (dia, conteudo)` | retrato diário |
| `bsq_meta (chave, valor)` | rev de cache, estado da sync do Omie, saldos |
| Storage bucket `bsq-arquivos` | anexos/PDFs em partes |
...
### 4.3 `bsq-omie` (a ponte com o ERP)
...
### 4.4 `bsq-acervo` (arquivos)
Upload/baixa em partes no bucket `bsq-arquivos` (anexos de venda, PDFs de proposta).
...
**Service worker** (`sw.js`): shell cache `bsq-shell-vN` — **toda publicação
...
1. **Hub Impresilk** (05:40): puxa `list` paginado via ROTINA e grava
   `bosques/AAAA-MM-DD.json` no repo privado `backups-impresilk`.
2. **bsq_backup** no Supabase (rotina própria; indicador em Configurações).
Restauração: action `restaurar` do nucleo aceita o formato do backup.

## 11. Segredos (nomes e moradas — valores NUNCA aqui)

No Supabase (projeto Projetos Léo): `BSQ_TOKEN` (leve, também no bundle),
`BSQ_PAINEL_SENHA` (senha-mestra inicial), `BSQ_ROTINA_TOKEN` (backup/portas
fortes), `BSQ_OMIE_APP_KEY` + `BSQ_OMIE_APP_SECRET` (painel do Omie),
`SB_SECRET_KEY` (service). Cópias de trabalho: pasta `seed/` local (gitignored).
No painel do hub: `SISTEMAS_BACKUP_BOSQUES` (registry com a rotina).
